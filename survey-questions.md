...
Welcome to the Price Statistics Production Systems Survey! This survey is concerned with the interaction between teams, software systems, and key processing steps of the Generic Statistical Business Process Model (GSBPM) used in production systems to make price indices, namely the Consumer Price Index (CPI).

As the UN Task Team on Scanner, we are administering this survey on behalf of UN United Nations Committee of Experts on Big Data and Data Science for Official Statistics (UN-CEBD) in order to understand how national statistical agencies use software to build their CPI. We look forward to share the results of this survey in upcoming Price Statistics conferences (such as the [Ottawa Group on Price Indices](https://stats.unece.org/ottawagroup/) and the [Expert group on Consumer Price Indices](https://unece.org/expert-group-consumer-price-indices)). Results of this survey will be shared in aggregated fashion unless you explicitly allow us to reference your example.

## Preamble and Definitions

This survey is concerned with the interaction between teams, software systems, and key processing steps of the Generic Statistical Business Process Model (GSBPM) used in production systems to make price indices, namely the Consumer Price Index (CPI).
...
| **Finalization** | Store price indices for subsequent use as part of analytical activities, and eventual dissemination. |
...
A system can handle a single step (shown in "Aggregation" and "Finalization"), multiple steps (shown in "Data Processing" and "Elementary indices"), or multiple systems can be involved in handling a single step (shown in "Data Ingestion").
...
This figure can be interpretted as follows. - A corporate IT team maintains a large Java application with an application-specific database that handles ingestion of most data sources. - The corporate IT team boxes are drawn in blue, indicating that they are a **central organization team** that does not necessarily have detailed **price statistics** **domain knowledge**. - In the absence of domain knowledge, this team would need to engage in a requirements gathering process with the domain team in order to build a system that performs the required functionalities.
...
-   One or more teams of Economists and Statisticians maintain a series of systems, using some combination of R and Python (or other language), that perform data processing and computation of elementary indices.
    ...
    -   The remaining teams have two systems to respectively handle data processing and calculating elementary indices.
-   A second corporate IT system aggregates the elementary indices and stores the resulting price indices in a database.
...
| Data ingestion | Data processing | Elementary indices | Aggregation | Finalization |
|---------------|---------------|---------------|---------------|---------------|
...
#### 4.1 For any of the GSBPM steps in this survey, do you use any open source libraries for making price indices as part of your system?
...
|--------|--------|--------|--------|--------|--------|--------|--------|--------|
| Ingestion | Select \# | Select \# | Select \# | Select \# | Select \# | Select \# | Select \# | Select \# |
| Validation | Select \# | Select \# | Select \# | Select \# | Select \# | Select \# | Select \# | Select \# |
| Elementals | Select \# | Select \# | Select \# | Select \# | Select \# | Select \# | Select \# | Select \# |
| Aggregation | Select \# | Select \# | Select \# | Select \# | Select \# | Select \# | Select \# | Select \# |
| Finalization | Select \# | Select \# | Select \# | Select \# | Select \# | Select \# | Select \# | Select \# |
